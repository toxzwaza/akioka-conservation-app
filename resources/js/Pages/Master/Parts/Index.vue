...
function destroy(id) {
    ...
    router.delete(route('master.parts.destroy', id));
}
</script>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">部品番号</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">部品名称</th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-200">
                            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-slate-800">{{ item.part_no }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-slate-800">{{ item.display_name ?? item.name }}</td>
